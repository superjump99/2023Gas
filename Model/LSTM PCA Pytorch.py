...
def build_dataset(time_series, seq_length):
    ...
    for i in range(0, len(time_series)-seq_length):
        ...
        _y = time_series[i+seq_length, -1]        # CH4 부분
        # print(_x, "-->",_y)
        dataX.append(_x)
        dataY.append(_y)
...
class Model(nn.Module):
    def __init__(self,feature_size,target_size, hidden_size,num_layers, dropout):
        ...
        self.lstm = nn.LSTM(input_size=feature_size, hidden_size=self.hidden_size, num_layers=self.num_layer,
                            # bidirectional=True,
                            batch_first=True)     # True =>[batch_size, seq_length, input_size]

        self.fc1 = nn.Linear(in_features=seq_length * self.hidden_size, out_features=30)      # seq_length * self.hidden_size => 모델에서 (시퀀스 길이 * hidden_size)로 출력
        self.relu = nn.ReLU()
    ...
    def forward(self, x):
        ...
        x, (hn,cn) = self.lstm(x, (h0,C0))   # RNN층의 출력
        # h0 = 초기 은닉 상태
...
if __name__ == '__main__':
    ...
    if is_train_mode:
        ...
        for i in range(100):
            ...
            pca = PCA(n_components=3)  # n_components=7
            pca.fit(train_set.iloc[:, 1:])
            # print(pca)
            pca_train_set = pca.transform(train_set.iloc[:, 1:])
            pca_test_set = pca.transform(test_set.iloc[:, 1:])

            scaled_train_set = pd.DataFrame(pca_train_set, index=train_set.index,
                                  columns=[f'pca{num + 1}' for num in range(3)])
            scaled_test_set = pd.DataFrame(pca_test_set, index=test_set.index,
                                         columns=[f'pca{num + 1}' for num in range(3)])
        # print(scaled_train_set)
        # exit()
        # train_set = pca_train_set
        # test_set = pca_test_set
        # train_set.iloc[:,1:] = pca.transform(train_set.iloc[:, 1:])
        # test_set.iloc[:, 1:] = pca.transform(test_set.iloc[:, 1:])
            # Output scale
            scaler_y = MinMaxScaler()
            scaler_y.fit(train_set.iloc[:, [0]])
            scaled_train_set['CH4'] = scaler_y.transform(train_set.iloc[:, [0]])
            scaled_test_set['CH4'] = scaler_y.transform(test_set.iloc[:, [0]])

            trainX, trainY = build_dataset(np.array(scaled_train_set), seq_length)
            testX, testY = build_dataset(np.array(scaled_test_set), seq_length)
            # 텐서로 변환
